...
#use below func if no of zones not equal to nodes

# def get_graphMatrix(network, nodes, od_paths_dict):
#     """
#     Return flattened OD-level matrix with averaged link features per OD.
#     """
#     n = len(nodes)
#     Cap_mat = np.zeros((n, n))
#     Length_mat = np.zeros((n, n))
#     Fft_mat = np.zeros((n, n))
...
#                 continue
#             od = (o, d)
#             links = od_paths_dict.get(od, [])
#             if len(links) == 0:
#                 continue
#             # --- Average the link features ---
#             cap_vals = network.loc[network.index.isin(links), 'capacity'].values
#             length_vals = network.loc[network.index.isin(links), 'length'].values
#             fft_vals = network.loc[network.index.isin(links), 'free_flow_time'].values

#             if len(cap_vals) > 0:
#                 Cap_mat[o-1, d-1] = cap_vals.mean()
#                 Length_mat[o-1, d-1] = length_vals.mean()
#                 Fft_mat[o-1, d-1] = fft_vals.mean()

#     # Flatten to column vectors
#     Cap = Cap_mat.reshape(-1,1)
#     Length = Length_mat.reshape(-1,1)
#     Fft = Fft_mat.reshape(-1,1)
...
# use the generate xy below if number of zones not equal to number of nodes
...
#     path_links = stat["data"]["paths_link"]
#     demand = stat["data"]["demand"]
#     path_flows = [to_percentage_list(inner_list) for inner_list in stat["path_flow"]]

#     all_nodes = stat["data"]["nodes"]
#     net = stat["data"]["network"]

#     # Use only the first 38 nodes as zones for Aneheim
#     nodes = list(all_nodes)[:38]

#     #print(f"⚙️ Using only first {len(nodes)} nodes as zones (1–38) out of {len(all_nodes)} total nodes.")

#     # --- Build X ---
    
#     # --- Build OD_paths dictionary ---
#     # Map each OD pair to the list of all links in all paths
#     od_paths_dict = {}
#     for od, paths in path_links.items():   # path_links assumed {(o,d): [[link1, link2,...], ...]}
#         od_paths_dict[od] = [link for path in paths for link in path]  # flatten all paths to links

#     # --- Build X using averaged graph features ---
#     Graph = get_graphMatrix(net, nodes, od_paths_dict)
...
def get_delay(path_flow, link_flow):
    ...
    path_flow['Nr'] = (
        (path_flow['path1_cost'] - path_flow['min_path_cost']) +
        (path_flow['path2_cost'] - path_flow['min_path_cost']) +
        (path_flow['path3_cost'] - path_flow['min_path_cost'])
